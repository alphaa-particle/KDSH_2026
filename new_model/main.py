...
# # # from train import TrackBHead  # same head class

# # # DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# # # DB_PATH = "C:\\Users\\vaibh\\Desktop\\KDSH_Solution_2026\\new_model\\my_novel_db"
...
# # # # wide retrieve, rerank, then belief-update on K
# # # WIDE_N = 80
# # # FINAL_K = 16
...
# # #     return torch.load(path, map_location="cpu")  # ids, offsets, embs
...
# # #         if fn.endswith(".pt") and fn.startswith(book.replace(" ", "_")):
# # #             obj = torch.load(os.path.join(ENSEMBLE_DIR, fn), map_location=DEVICE)
...
# # #     if not models:
# # #         raise FileNotFoundError(f"No ensemble checkpoints for book={book} in {ENSEMBLE_DIR}")
# # #     # average thresholds too (works well)
# # #     thr = sum(thresholds) / len(thresholds)
# # #     return models, thr
...
# # #         tfidf = book_res[book]["tfidf"]
# # #         chunks = book_res[book]["chunks"]
# # #         models = book_res[book]["models"]
# # #         thr = book_res[book]["thr"]

# # #         # ---- Stage 1: wide BDH vector retrieve (union over multi-queries) ----
# # #         cand_ids = set()
# # #         cand_docs = {}
# # #         cand_meta = {}
...
# # #                             # REMOVED "ids" from the list below
...
# # #         if not cand_ids:
# # #             preds.append("consistent")
# # #             continue

# # #         # ---- Stage 2: TF-IDF rerank within candidates (lexical constraints) ----
# # #         qfull = f"CHAR: {row['char']}\nCAPTION: {row.get('caption','')}\nBACKSTORY: {row.get('content','')}"
# # #         qv = tfidf["vectorizer"].transform([qfull])
# # #         qv = sk_normalize(qv, axis=1)

# # #         # map candidate ids -> tfidf row index
...
# # #         q = bdh_embed_text(lm, tok, qfull, DEVICE, max_len=512, windows=3, pool="mean")
# # #         q_cpu = q.detach().cpu()

# # #         E_cache = torch.nn.functional.normalize(chunks["embs"], dim=-1)  # cpu
...
# # #         cT = torch.stack(ev, dim=0).to(DEVICE)               # (K,D)
# # #         sT = torch.tensor(sims, dtype=torch.float32, device=DEVICE)  # (K,)
...
# # #         preds.append("consistent" if p >= thr else "inconsistent")
...
# # def load_ensemble(book: str, d: int):
# #     """
# #     Loads all K-Fold models for a specific book and calculates the
# #     consensus threshold (average of all fold thresholds).
# #     """
# #     models = []
# #     thresholds = []
    
# #     # We clean the book name to match how it was saved in train.py
# #     # e.g. "The Count..." -> "thecount..." or "The_Count..." depending on your slug logic
# #     # In train.py we used book.replace(' ', '_'), so let's match that strictly if possible,
# #     # OR rely on the standard slug if that's what you used for files.
# #     # Assuming files are named like: "In_Search_of_the_Castaways_fold0.pt"
# #     safe_book_prefix = book.replace(" ", "_")
    
# #     print(f"Loading ensemble for: {book} (looking for prefix '{safe_book_prefix}' in {ENSEMBLE_DIR})")
...
# #     # Average thresholds for voting
# #     avg_thr = sum(thresholds) / len(thresholds)
# #     print(f"   > Loaded {found_count} models. Consensus Threshold: {avg_thr:.3f}")
# #     return models, avg_thr
...
# #         if res_pack is None:
# #              preds.append("consistent") # Default fallback
# #              continue
...
# #                 n_results=WIDE_N,
# #                 where={"book_name": book},
# #                 # FIX IS HERE: Removed "ids" from this list
# #                 include=["documents", "metadatas"], 
# #             )
            
# #             # The 'ids' key is always present in 'res' by default
...
# #         # ---- Ensemble predict (Soft Voting) ----
# #         with torch.no_grad():
# #             ps = []
# #             for m in models:
# #                 # Get prob from each fold's expert
# #                 p = torch.sigmoid(m(q.unsqueeze(0), cT.unsqueeze(0), sT.unsqueeze(0)))[0].item()
# #                 ps.append(p)
...
# #         # Compare average probability to consensus threshold
# #         preds.append("consistent" if avg_p >= thr else "inconsistent")
...
# WIDE_N = 80
# FINAL_K = 16
# MIN_THRESHOLD = 0.40  # <--- FORCE STRICTNESS (Fixes the "Only Consistent" bug)
...
#     # --- CRITICAL FIX: Clamp the threshold ---
#     # If the training learned 0.05, we force it up to MIN_THRESHOLD
#     # This prevents the model from marking everything "Consistent" just because it's unsure.
...
# # --- CRITICAL FIX: Match the training feature generation EXACTLY ---
...
#     # 1. Embed the base text
#     q = bdh_embed_text(lm, tok, base, DEVICE, max_len=512, windows=3, pool="mean")
    
#     # 2. Embed the claims (This was MISSING in your previous main.py)
...
#         # ---- Stage 1: wide BDH vector retrieve ----
#         cand_ids = set()
#         cand_docs = {}
#         cand_meta = {}
...
#         # Use simple text for TF-IDF (no need for complex embedding logic here)
...
#         # ---- Stage 3: Evidence & Prediction ----
#         cache_ids = chunks["ids"]
#         cache_map = {cid: j for j, cid in enumerate(cache_ids)}
#         ev = []
#         sims = []

#         # --- FIX: Use make_query_emb to match training ---
...
#         if i % 50 == 0:
#             print(f"Processed {i}/{len(df)} | Last Prob: {avg_p:.3f} Thr: {thr:.3f}")
...
#         prediction = "consistent" if avg_p >= thr else "inconsistent"
