...
#             idxs = list(range(len(dbook)))
#             rnd.shuffle(idxs)

#             # simple split inside book (stable)
#             split = max(1, int(0.8 * len(idxs)))
#             tr, va = idxs[:split], idxs[split:]

#             head = TrackBHead(cfg.n_embd, DROPOUT).to(DEVICE)
#             opt = AdamW(head.parameters(), lr=LR)

#             # class weight inside book
#             pos = float(dbook.loc[tr, "y"].sum())
...
#                         probs.append(p); yt.append(int(row["y"]))
...
#             out = os.path.join(ENSEMBLE_DIR, f"{book.replace(' ','_')}_seed{seed}.pt")
#             torch.save(
#                 {"book": book, "bdh_config": cfg.__dict__, "head_state": best_state, "threshold": best_t,
#                  "topk": TOPK_EVIDENCE, "dropout": DROPOUT, "seed": seed},
#                 out,
#             )
#             print(f"✅ Saved {out} best_f1={best_f1:.3f} thr={best_t:.2f}")
