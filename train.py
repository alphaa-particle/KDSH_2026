...
#         pred_str = "Consistent" if all_preds[i] == 1 else "Inconsistent"
#         true_str = "Consistent" if all_labels[i] == 1 else "Inconsistent"
#         match = "✅" if pred_str == true_str else "❌"
#         print(f"     Example {i+1}: Pred: {pred_str} | Real: {true_str} {match}")
#     print("   ------------------------------")
...
#             print(f"   ✅ New Best Model Saved! (Highest F1: {val_f1*100:.2f}%)")
...
def train():
    ...
    train_real_df, val_df = train_test_split(real_df, test_size=0.2, random_state=42)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1} | Train Loss: {avg_train_loss:.4f} | Val F1: {val_f1:.4f} | Acc: {val_acc:.4f}")
